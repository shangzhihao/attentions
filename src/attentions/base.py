...
class BaseSelfAttention(nn.Module, ABC):
    ...
    def _get_rope_freqs(
        ...
        dim_pairs = d_head // 2
        freqs = 1.0 / (
            10000.0 ** (torch.arange(0, dim_pairs, dtype=dtype, device=device) * 2.0 / d_head)
        )
    ...
    def extra_repr(self) -> str:
        ...
        return (
            f"d_model={self.d_model}, input_dim={self.input_dim}, "
            f"dropout={self.dropout_prob}, bias={self.bias}, rope={self.rope}"
        )
