...
def test_vanilla_forward_basic_shapes():
...
def test_vanilla_forward_flexible_input_dimensions():
...
def test_vanilla_forward_with_causal_mask():
...
def test_vanilla_forward_with_padding_mask():
    ...
    assert (attention_weights[:, :, -2:] < 1e-6).all()


def test_vanilla_forward_attention_weights_properties():
...
def test_vanilla_forward_temperature_scaling():
...
def test_vanilla_forward_deterministic_behavior():
...
def test_vanilla_forward_gradient_flow():
...
def test_vanilla_forward_batch_independence():
...
def test_vanilla_forward_sequence_length_variation():
...
def test_vanilla_forward_dtype_preservation():
...
def test_vanilla_forward_edge_cases():
    ...
    assert torch.allclose(weights_small.sum(dim=-1), torch.ones(1, 3), atol=1e-6)