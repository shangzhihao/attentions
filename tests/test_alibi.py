...
def test_alibi_forward_basic_shapes():
...
def test_alibi_forward_flexible_input_dimensions():
...
def test_alibi_slopes_computation():
...
def test_alibi_bias_creation():
    ...
    assert attention.alibi_bias.shape == (num_heads, max_seq_len, max_seq_len)
    
    # Check that bias is zero on diagonal
    for head in range(num_heads):
        diagonal = torch.diag(attention.alibi_bias[head])
        assert torch.allclose(diagonal, torch.zeros_like(diagonal))
    
    # Check that bias is symmetric and negative
    for head in range(num_heads):
        bias_matrix = attention.alibi_bias[head]
    ...
    for head in range(num_heads):
        ...
        expected_bias_01 = -attention.slopes[head] * 1
        assert torch.allclose(
            attention.alibi_bias[head, 0, 1], 
            expected_bias_01, 
            atol=1e-6
        )
        
        # Position (0, 3) should have bias = -slope * 3
        expected_bias_03 = -attention.slopes[head] * 3
        assert torch.allclose(
            attention.alibi_bias[head, 0, 3], 
            expected_bias_03, 
            atol=1e-6
        )


def test_alibi_forward_different_head_counts():
...
def test_alibi_forward_with_causal_mask():
...
def test_alibi_forward_with_batch_mask():
...
def test_alibi_forward_long_sequences():
...
def test_alibi_forward_position_bias_effect():
...
def test_alibi_forward_temperature_scaling():
...
def test_alibi_forward_initialization_errors():
